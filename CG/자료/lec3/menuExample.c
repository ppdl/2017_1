...
// Call back registered for root menu
void
...
// Call back registered for submenu
void
subMenuCallback(int menuItem)
{
  switch (menuItem)
  { 
...
int main(void)
{
   ...
   glutCreateWindow ("menuExample (lec3)");
  ...
  subMenu=glutCreateMenu(subMenuCallback);      // Each sub menu can have a call back
  ...
  glutAttachMenu(GLUT_RIGHT_BUTTON);            // Attach the menu to the RIGHT_BUTTON
                                                // NOTE THAT MOUSE FUNC for RIGHT CLICK WILL NOT BE
                                                // CALLED NOW
